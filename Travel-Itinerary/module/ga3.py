...
class ga(threading.Thread):
	...
	def geneticAlgorithm(self, population):
		...
		population.insert(0, self.origin)

		response = gmaps.distance_matrix(origins=population, destinations=population)
		for i in response['rows']:
			for j in i['elements']:
				distance_val.append(j['distance']['value'])
				durations.append(j['duration']['value'])

		durArr = self.split(durations, len(list_addr))
		distArr = self.split(distance_val, len(list_addr))
